...
def remove_habit_completion(
    db: Session, habit_id: int, user_id: int, completed_on: date
) -> bool:
...
def get_mood_log(db: Session, mood_id: int, user_id: int) -> Optional[models.MoodLog]:
    ...
    return (
        ...
        .filter(models.MoodLog.mood_id == mood_id, models.MoodLog.user_id == user_id)
        .first()
    )


def get_mood_log_by_date(
    db: Session, user_id: int, logged_on: date
) -> Optional[models.MoodLog]:
    """Get a mood log for a specific date."""
    return (
        db.query(models.MoodLog)
        .filter(
            models.MoodLog.user_id == user_id, models.MoodLog.logged_on == logged_on
        )
        .first()
    )
...
def get_user_mood_logs(
    ...
    return (
        query.order_by(models.MoodLog.logged_on.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def get_mood_statistics(db: Session, user_id: int, days: int = 30) -> dict:
    ...
    all_logs = get_user_mood_logs(
        db, user_id=user_id, limit=365  # Check up to a year for streaks
    )
