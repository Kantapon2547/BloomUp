...
def check_gratitude_achievements(db: Session, user_id: int):
    ...
    gratitude_count = db.query(models.GratitudeEntry).filter(
        models.GratitudeEntry.user_id == user_id
    ).count()
    
    # Check Gratitude Pro (10 entries)
    gratitude_pro = db.query(models.UserAchievement).filter(
        models.UserAchievement.user_id == user_id,
        models.UserAchievement.achievement.has(models.Achievement.key_name == "gratitude_pro")
    ).first()
...
def check_habit_achievements(db: Session, user_id: int):
    ...
    habit_count = db.query(models.Habit).filter(
        models.Habit.user_id == user_id,
        models.Habit.is_active == True
    ).count()
    
    # Check First Steps (1 habit created)
    first_steps = db.query(models.UserAchievement).filter(
        models.UserAchievement.user_id == user_id,
        models.UserAchievement.achievement.has(models.Achievement.key_name == "first_steps")
    ).first()
    ...
    habit_collector = db.query(models.UserAchievement).filter(
        models.UserAchievement.user_id == user_id,
        models.UserAchievement.achievement.has(models.Achievement.key_name == "habit_collector")
    ).first()
...
def check_streak_achievements(db: Session, user_id: int):
    ...
    ).order_by(models.HabitCompletion.completed_on.desc()).all()
    ...
    streak_master = db.query(models.UserAchievement).filter(
        models.UserAchievement.user_id == user_id,
        models.UserAchievement.achievement.has(models.Achievement.key_name == "streak_master")
    ).first()
...
def check_mood_achievements(db: Session, user_id: int):
    ...
    """
    # Get mood log count
    mood_count = db.query(models.MoodLog).filter(
        models.MoodLog.user_id == user_id
    ).count()
    
    # Check Mood Tracker (20 mood logs)
    mood_tracker = db.query(models.UserAchievement).filter(
        models.UserAchievement.user_id == user_id,
        models.UserAchievement.achievement.has(models.Achievement.key_name == "mood_tracker")
    ).first()
